...
function getPolynom(...argum) {
  ...
  const args = Array.from(argum).reverse();
  const koefOne = args[0] || 0;
  const koefTwo = args[1] || 0;
  const koefThree = args[2] || 0;
  return (x) => koefThree * x ** 2 + koefTwo * x + koefOne;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const logStart = `${func.name}(${args.map(JSON.stringify).join(',')}) starts`;
    logFunc(logStart);
    const result = func(...args);
    const logEnd = `${func.name}(${args.map(JSON.stringify).join(',')}) ends`;
    logFunc(logEnd);
    return result;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  ...
  return () => {
    if (counters.has(startFrom)) {
      counters.set(startFrom, counters.get(startFrom) + 1);
    } else {
      counters.set(startFrom, startFrom);
    }
    return counters.get(startFrom);
  };
}
