...
/**
   ...
   *
   */
function processAllPromises(arrays) {
...
function chainPromises(array, action) {
  ...
  let chain;
  array.forEach((item) => {
    chain = item // assigned to promise
      .then((resolvedValue) => {
        result.push(resolvedValue);
      })
      .catch(() => {});
  });
  return chain.then(() => result.reduce(action));
  // referring result array directrly would give an empty array
}
